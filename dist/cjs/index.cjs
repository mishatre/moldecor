...
var require_lodash = __commonJS({
  "node_modules/.pnpm/lodash.defaultsdeep@4.6.1/node_modules/lodash.defaultsdeep/index.js"(exports2, module2) {
...
var uniqWith = (arr, fn) => arr.filter((element, index) => arr.findIndex((step) => fn(element, step)) === index);
...
function mergeSchemaSettings(src, target) {
  ...
  return (0, import_lodash.default)(src, target);
}
function mergeSchemaMetadata(src, target) {
  return (0, import_lodash.default)(src, target);
}
function mergeSchemaHooks(src, target) {
  if (!target) {
    target = {};
  }
  if (src) {
    for (const key of Object.keys(src)) {
      ...
      for (const hookKey of Object.keys(srcValue)) {
        ...
        target[key][hookKey] = flatten(key === "before" ? [resHook, modHook] : [modHook, resHook]).filter(Boolean);
      }
    }
    ;
  }
...
function mergeSchemaActions(src, target) {
  if (!target) {
    target = {};
  }
  if (src) {
    for (const key of Object.keys(src)) {
      ...
      if (typeof srcAction !== "boolean" && (srcAction == null ? void 0 : srcAction.hooks) && typeof targetAction !== "boolean" && (targetAction == null ? void 0 : targetAction.hooks)) {
        for (const hookKey of Object.keys(srcAction.hooks)) {
          const modHook = wrapToArray(srcAction.hooks[hookKey]);
          const resHook = wrapToArray(targetAction.hooks[hookKey]);
          srcAction.hooks[hookKey] = flatten(hookKey === "before" ? [resHook, modHook] : [modHook, resHook]).filter(Boolean);
        }
        ;
      }
      if (srcAction === false) {
...
function mergeSchemaMethods(src, target) {
  return Object.assign(target || {}, src || {});
}
function mergeSchemaEvents(src, target) {
  if (!target) {
    target = {};
  }
  if (src) {
    for (const key of Object.keys(src)) {
      ...
      let handler = flatten([resEvent ? resEvent.handler : null, modEvent ? modEvent.handler : null]).filter(Boolean);
      target[key] = (0, import_lodash.default)(modEvent, resEvent);
      target[key].handler = handler.length === 1 ? handler[0] : handler;
    }
  }
  ;
  return target;
}
function mergeSchemaLifecycleHandlers(src, target) {
  return flatten([target, src]).filter(Boolean);
}
function mergeSchemaUniqArray(src, target) {
  return uniqWith(flatten([src, target]).filter(Boolean), (a, b) => JSON.stringify(a) === JSON.stringify(b));
}
function mergeSchemaUnknown(src, target) {
  if (src !== void 0) {
    return src;
  }
  return target;
...
function service(options) {
  return function(target, context) {
    (0, import_node_assert2.default)(context.kind === "class", "Service decorator can be used only as class decorator");
