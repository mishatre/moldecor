...
export interface ServiceOptions<S extends Record<string, any>> {
	name?: string;
...
>(options: ServiceOptions<S>) {
	return (
		target: T & { [decoratedService]?: Partial<ServiceSchema> },
		context: ClassDecoratorContext<T>,
	) => {
		assert(
			context.kind === "class",
			"Service decorator can be used only as class decorator",
		);
		...
		if ("mixins" in metadata && Array.isArray(metadata.mixins)) {
			metadata.mixins = metadata.mixins.map((mixin) =>
				decoratedService in mixin
					? (mixin[decoratedService] as Partial<ServiceSchema>)
					: mixin,
			);
		}

		if (decoratedFields in metadata && metadata[decoratedFields]) {
			const merged = function <S extends Service>(
				this: S,
				schema: ServiceSchema,
			) {
				if (decoratedFields in metadata && metadata[decoratedFields]) {
					for (const [key, value] of Object.entries(
						metadata[decoratedFields],
					)) {
						if (!(key in this)) {
...
>(params: P) {
	return (handler: T, context: ClassMethodDecoratorContext<S, T>) => {
		assert(
			context.kind === "method",
...
export function method<S, T extends (...args: any[]) => any>(
	handler: T,
	context: ClassMethodDecoratorContext<S, T>,
) {
	assert(
		context.kind === "method",
...
export function created<S, T extends (...args: any) => any>(
	handler: T,
	context: ClassMethodDecoratorContext<S, T>,
) {
	assert(
...
export function started<S, T extends (...args: any) => any>(
	handler: T,
	context: ClassMethodDecoratorContext<S, T>,
) {
	assert(
...
export function stopped<S, T extends (...args: any) => any>(
	handler: T,
	context: ClassMethodDecoratorContext<S, T>,
) {
	assert(
...
export function merged<S, T extends (...args: any) => any>(
	handler: T,
	context: ClassMethodDecoratorContext<S, T>,
) {
	assert(
...
export function lifecycle<S, T extends (...args: any) => any>(
	handler: T,
	context: ClassMethodDecoratorContext<S, T>,
) {
	assert(
	...
	assert(
		["created", "merged", "started", "stopped"].includes(
